...
for n=1:number_of_fits
    ...
    if ~strcmp(data_order,'xyzfile')
        ...
        image_3d = nii.img;
%         input_hdr = nii.hdr;
        if isfield(nii.hdr.dime,'scl_slope')
            %                 image_3d = double(image_3d)./nii.hdr.dime.scl_slope;
            image_3d = double(image_3d).*nii.hdr.dime.scl_slope;
        end
        if isfield(nii.hdr.dime,'scl_inter')
            %                 image_3d = double(image_3d)-nii.hdr.dime.scl_inter;
            image_3d = double(image_3d)+nii.hdr.dime.scl_inter;
        end
        if isfield(nii.hdr.dime,'scl_slope')
        ...
        for m=1:dim_n
            ...
            [file_path, filename]  = fileparts(imagefile);
            nii = load_untouch_nii(imagefile);
            res = nii.hdr.dime.pixdim; %nii.hdr.dime.scl_slope, nii.hdr.dime.scl_inter
            res = res(2:4);
            image_3d = nii.img;
%             input_hdr = nii.hdr;
            if isfield(nii.hdr.dime,'scl_slope')
